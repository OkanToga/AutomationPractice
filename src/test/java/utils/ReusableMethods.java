...
//   waitFor(5);  => waits for 5 second
    ...
    public static void selectRandomDateOfBirth(WebElement dayElement, WebElement monthElement, WebElement yearElement) {
        // Gün seçimi
        Select selectDay = new Select(dayElement);
        int randomDayIndex = random().nextInt(selectDay.getOptions().size() - 1) + 1;
        selectDay.selectByIndex(randomDayIndex);

        // Ay seçimi
        Select selectMonth = new Select(monthElement);
        int randomMonthIndex = random().nextInt(selectMonth.getOptions().size() - 1) + 1;
        selectMonth.selectByIndex(randomMonthIndex);

        // Yıl seçimi
        Select selectYear = new Select(yearElement);
        int randomYearIndex = random().nextInt(selectYear.getOptions().size() - 1) + 1;
        selectYear.selectByIndex(randomYearIndex);
    }
    public static void selectSpecificDate(WebElement dayElement, String day, WebElement monthElement, String month, WebElement yearElement, String year) {
        Select daySelect = new Select(dayElement);
        daySelect.selectByValue(day); // Örneğin, "16"

        Select monthSelect = new Select(monthElement);
        monthSelect.selectByValue(month); // Örneğin, "1" (Ocak)

        Select yearSelect = new Select(yearElement);
        yearSelect.selectByValue(year); // Örneğin, "2002"
    }
